...
:return {*}
'''
def calculate_loss(enclosing_boxes:torch.Tensor, predict:torch.Tensor, gt_boxes:torch.Tensor):
    '''
    ...
    # 计算每个BB的IOU
    IOU_metrix[IOU_metrix < THRESHOLD_GT] = 0
    # 对每个最小外接框找到最佳匹配的GT
    ...
    for i in range(len(gt_box_index)):
        ...
        # predict_width = enclosing_box_width*torch.exp(predict[i,2])
        # predict_height = enclosing_box_height*torch.exp(predict[i,3])
        # predict_center_x = enclosing_box[0] + enclosing_box_width*predict[i,0:2].softmax(dim=0)[0]
        # predict_center_y = enclosing_box[1] + enclosing_box_height*predict[i,0:2].softmax(dim=0)[1]
        ...
        # predict_box = torch.tensor([predict_center_x-predict_width/2, predict_center_y-predict_height/2, 
        #                             predict_center_x+predict_width/2, predict_center_y+predict_height/2])
        loss += complete_box_iou_loss(gt_box, predict_box)
    return loss
